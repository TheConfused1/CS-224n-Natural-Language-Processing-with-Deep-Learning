...
def naiveSoftmaxLossAndGradient(
    ...
    #prob = np.exp(np.dot(outsideVectors[outsideWordIdx],centerWordVec))/( sum( np.exp(np.matmul(outsideVectors.T,centerWordVec))))
    #loss=-np.loss(prob)
    ...
    coeff= prob
    coeff[outsideWordIdx] -= 1                     #holds only for the context word being considered
    gradCenterVec = np.matmul(coeff, outsideVectors)

    gradOutsideVecs=np.array([(p)*centerWordVec for p in prob])
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    #print(centerWordVectors.shape,centerWordVectors)
    centerWordVec = centerWordVectors[word2Ind[currentCenterWord],:]

    outsideIndicies = [ word2Ind[word] for word in outsideWords ]
    #print(outsideWords,outsideIndicies)

    for idx in outsideIndicies:
        temp_loss, gradCenterVec, gradOutsideVecs = word2vecLossAndGradient(centerWordVec, idx, outsideVectors, dataset)
        loss+=temp_loss
